...
export default async function handler(req, res) {
  try {
    ...
    console.log('[initiate-payment] Price API raw response:', priceText)

    if (!priceRes.ok) {
      return res.status(500).json({ error: `Smoobu pricing error: ${priceText}` })
    }

    const { totalPrice } = JSON.parse(priceText)
